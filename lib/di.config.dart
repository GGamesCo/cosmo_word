// GENERATED CODE - DO NOT MODIFY BY HAND

// **************************************************************************
// InjectableConfigGenerator
// **************************************************************************

import 'package:get_it/get_it.dart' as _i1;
import 'package:injectable/injectable.dart' as _i2;
import 'package:shared_preferences/shared_preferences.dart' as _i12;

import 'Analytics/AnalyticsController.dart' as _i18;
import 'Flame/Common/SoundsController.dart' as _i13;
import 'GameBL/Common/Abstract/IBalanceController.dart' as _i3;
import 'GameBL/Common/Abstract/IFlowRepository.dart' as _i5;
import 'GameBL/Common/Abstract/ITimerController.dart' as _i7;
import 'GameBL/Common/Abstract/IWordInputController.dart' as _i19;
import 'GameBL/Common/Abstract/IWordRepository.dart' as _i9;
import 'GameBL/Common/BalanceController.dart' as _i4;
import 'GameBL/Common/FlowRepository.dart' as _i6;
import 'GameBL/Common/StageManager.dart' as _i22;
import 'GameBL/Common/TimerController.dart' as _i8;
import 'GameBL/Common/UserController.dart' as _i16;
import 'GameBL/Common/WordInputController.dart' as _i20;
import 'GameBL/Common/WordRepository.dart' as _i10;
import 'GameBL/DI/Module.dart' as _i25;
import 'GameBL/Lobby/LobbyStage.dart' as _i21;
import 'GameBL/Services/StoryLevelsService/StoryLevelsService.dart' as _i14;
import 'GameBL/Services/StoryLocationsService/StoryLocationsService.dart'
    as _i15;
import 'GameBL/Services/UserStateService/UserStateService.dart' as _i17;
import 'GameBL/TimeChallenge/RocketChallengeConfig.dart' as _i11;
import 'GameBL/TimeChallenge/TimeAtackStage.dart' as _i24;
import 'GameBL/UserStateController.dart'
    as _i23; // ignore_for_file: unnecessary_lambdas

// ignore_for_file: lines_longer_than_80_chars
/// initializes the registration of provided dependencies inside of [GetIt]
_i1.GetIt $initGetIt(_i1.GetIt get,
    {String? environment, _i2.EnvironmentFilter? environmentFilter}) {
  final gh = _i2.GetItHelper(get, environment, environmentFilter);
  final registerModule = _$RegisterModule();
  gh.singleton<_i3.IBalanceController>(_i4.BalanceController());
  gh.factory<_i5.IFlowRepository>(() => _i6.FlowRepository());
  gh.factory<_i7.ITimerController>(() => _i8.TimerController());
  gh.factory<_i9.IWordRepository>(() => _i10.WordRepository());
  gh.factory<_i11.RocketChallengeConfig>(() => _i11.RocketChallengeConfig());
  gh.factoryAsync<_i12.SharedPreferences>(() => registerModule.prefs);
  gh.singleton<_i13.SoundsController>(_i13.SoundsController());
  gh.singleton<_i14.StoryLevelsService>(_i14.StoryLevelsService());
  gh.singleton<_i15.StoryLocationsService>(_i15.StoryLocationsService());
  gh.singleton<_i16.UserController>(_i16.UserController());
  gh.singletonAsync<_i17.UserStateService>(() async => _i17.UserStateService(
      sharedPreferences: await get.getAsync<_i12.SharedPreferences>()));
  gh.singleton<_i18.AnalyticsController>(
      _i18.AnalyticsController(userController: get<_i16.UserController>()));
  gh.factory<_i19.IWordInputController>(() => _i20.WordInputController(
      wordRepository: get<_i9.IWordRepository>(),
      balanceController: get<_i3.IBalanceController>(),
      analyticsController: get<_i18.AnalyticsController>()));
  gh.factory<_i21.LobbyStage>(
      () => _i21.LobbyStage(soundsController: get<_i13.SoundsController>()));
  gh.singleton<_i22.StageManager>(
      _i22.StageManager(analyticsController: get<_i18.AnalyticsController>()));
  gh.singletonAsync<_i23.UserStateController>(() async =>
      _i23.UserStateController(
          userStateService: await get.getAsync<_i17.UserStateService>(),
          levelsService: get<_i14.StoryLevelsService>(),
          flowRepository: get<_i5.IFlowRepository>(),
          balanceController: get<_i3.IBalanceController>(),
          analyticsController: get<_i18.AnalyticsController>()));
  gh.factoryAsync<_i24.TimeAtackStage>(() async => _i24.TimeAtackStage(
      userStateController: await get.getAsync<_i23.UserStateController>(),
      wordRepository: get<_i9.IWordRepository>(),
      wordInputController: get<_i19.IWordInputController>(),
      timerController: get<_i7.ITimerController>(),
      challengeConfig: get<_i11.RocketChallengeConfig>(),
      balanceController: get<_i3.IBalanceController>(),
      soundsController: get<_i13.SoundsController>()));
  return get;
}

class _$RegisterModule extends _i25.RegisterModule {}
