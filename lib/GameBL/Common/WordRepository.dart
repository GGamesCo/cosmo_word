...
class WordRepository extends IWordRepository{
  ...
  List<WordSet> sets = [
    ...
    WordSet(id: Uuid().v1(), size: 5, chars: ["C", "L", "O", "U", "D"], words: ["CL", "LO"/*"CLOUD", "LOUD", "COLD", "DOC"*/]),
  ...
  Future<WordSet> getSetAsync(int setSize, WordSet? exclude) async {
    Completer completerLoadingStub = Completer();
    completerLoadingStub.complete();
    await completerLoadingStub.future;

    return sets.firstWhere((element) => element.size == setSize && (exclude != null ? element.id != exclude.id: true));
  }
}